...
// End of Spell Cast Hook
    ...
    int nCasterLevel = GetTotalLevels(OBJECT_SELF, 1); //GetCasterLevel(OBJECT_SELF);
        ...
        if  ((nRacial == RACIAL_TYPE_ANIMAL || nRacial == RACIAL_TYPE_BEAST))
        {
           ...
           if (!MyResistSpell(OBJECT_SELF, oTarget))
    	   {
                ...
                if (!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_MIND_SPELLS))
                {
                    ...
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDom, oTarget, RoundsToSeconds(nDuration));
                }
            }
        }
    }
