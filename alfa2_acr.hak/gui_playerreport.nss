...
void main(int nAction, int nTargetObject)
{
    ...
    if(nAction == PLAYER_REPORT_SHOW_INVENTORY)
    {
        ...
        int nWealth = GetGold(oTarget);
				AddListBoxRow(OBJECT_SELF, "SCREEN_INVENTORYREPORT", "inventoryreport", "nw_it_gold", "LISTBOX_ITEM_TEXT=  Gold;LISTBOX_ITEM_PRICE=  "+IntToString(GetGold(oTarget))+";LISTBOX_ITEM_LEVEL=  1",   "LISTBOX_ITEM_ICON=it_gold.tga", "5="+IntToString(ObjectToInt(OBJECT_INVALID)), "unhide");
        int nCount = 0;
