...
void main(int iVarId) {    
	...
	SetGUIObjectText(oSubject, WAND_GUI_LV_MANAGER, "VAR_STR_INPUT", -1, sVarValue);
	
	if (iVarId >= 0) 
		SetGUIObjectDisabled(oSubject, WAND_GUI_LV_MANAGER, "DELETE_STR", FALSE);	
	else
		SetGUIObjectDisabled(oSubject, WAND_GUI_LV_MANAGER, "DELETE_STR", TRUE);	
}