...
void main()
{
	...
	{
		 SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, SPELL_ORB_OF_FIRE));
		 ...
		 // Orb spells are not resisted!!
			...
			// Savint throw for dazed effect
			if(!MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC()))
			{
				// failed saving throw, uh oh! Dazed!!
				effect eDazed = EffectDazed();
				ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDazed, oTarget, RoundsToSeconds(1));
			}
