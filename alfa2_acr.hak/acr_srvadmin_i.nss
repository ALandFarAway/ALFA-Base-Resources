...
int ACR_SrvAdmin_OnChat(object oPC, string sCmd)
{
	...
	else
	{
		...
		ACR_SrvAdmin_SendFeedback(oPC, " - rs <scriptlet> : Run a PowerShell/NWScript scriptlet.  The $s object can be used to access NWScript functions, like $s.SendMessageToPC($OBJECT_SELF, 'Message');.  The $db object (an ALFA.Database) can be used to make database accesses.");
